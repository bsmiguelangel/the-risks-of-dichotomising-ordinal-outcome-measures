...
# Western Europe data
country <- "west"
# ESSData$cntry[ESSData$cntry == "GB"] <- "UK"
...
# ESS cleaned
ESS <- ESSData[, c("polintr", "fnsdfml", "gincdif","health",  "wrclmch", 
                   "region", "gndr", "agecut", "educut", 
...
# Tell me how often you and your family experienced severe financial difficulties 
# when you were growing up?
# 1 = Always, 2 = Often, 3 = Sometimes, 4 = Hardly ever, 5 = Never,
# 7 = Refusal, 8 = Don't know, 9 = No answer
table(ESS$fnsdfml)
y <- as.numeric(ESS$fnsdfml)
y[y > 5] <- NA
y <- 6 - y
variable <- "fnsdfml"
y_labels <- c("Never", "Hardly ever", "Sometimes", "Often", "Always")

#### gincdif - Government should reduce differences in income levels ####

# Say to what extent you agree or disagree with the following statement:
...
# 7 = Refusal, 8 = Don't know, 9 = No answer
table(ESS$gincdif)
y <- as.numeric(ESS$gincdif)
y[y > 5] <- NA
variable <- "gincdif"
y_labels <- c("Disagree strongly", "Disagree", "Neither agree nor disagree", "Agree", "Agree strongly")
...
# 1 = Not at all worried, 2 = Not very worried, 3 = Somewhat worried,
# 4 = Very worried, 5 = Extremely worried, 6 = Not applicable,
# 7 = Refusal, 8 = Don't know, 9 = No answer
table(ESS$wrclmch)
y <- as.numeric(ESS$wrclmch)
y[y > 5] <- NA
variable <- "wrclmch"
y_labels <- c("Not at all worried", "Not very worried", "Somewhat worried", "Very worried", "Extremely worried")

#### polintr - How interested in politics ####

# How interested would you say you are in politics - are you...
# 1 = Very interested, 2 = Quite interested, 3 = Hardly interested,
# 4 = Not at all interested, 7 = Refusal, 8 = Don't know, 9 = No answer
table(ESS$polintr)
y <- as.numeric(ESS$polintr)
y[y > 4] <- NA
variable <- "polintr"
y_labels <- c("Very interested", "Quite interested", "Hardly interested", "Not at all interested")
...
ggsave(file.path("figures", country, variable, paste0("descriptive_", Gender_Cat, ".png")), 
...
saveRDS(salnimble, file = file.path("results", country, variable, paste0("ordinal-leroux-hmc-waic-", gender, ".rds")))
...
# # Option 1 (D+): 0 = First two categories; 1 = Last two categories.
# y[y > 4] <- NA
# y[y == 1 | y == 2] <- 0
# y[y == 3 | y == 4] <- 1
...
saveRDS(salnimble, file = file.path("results", country, variable, paste0("bernoulli-leroux-hmc-waic-", gender, "-o1.rds")))
...
# # Option 2 (D-): 0 = First three categories; 1 = Last category.
# y[y > 4] <- NA
# y[y == 1 | y == 2 | y == 3] <- 0
# y[y == 4] <- 1
...
saveRDS(salnimble, file = file.path("results", country, variable, paste0("bernoulli-leroux-hmc-waic-", gender, "-o2.rds")))

#### Model results ####

# Ordinal regression
ordinal_results <- readRDS(file = file.path("results", country, variable, paste0("ordinal-leroux-hmc-waic-", gender, ".rds")))
# Logistic regression (Option 1): 0 = First two categories; 1 = Last three categories.
bernoulli_results1 <- readRDS(file = file.path("results", country, variable, paste0("bernoulli-leroux-hmc-waic-", gender, "-o1.rds")))
# Logistic regression (Option 2): 0 = First three categories; 1 = Last two categories.
bernoulli_results2 <- readRDS(file = file.path("results", country, variable, paste0("bernoulli-leroux-hmc-waic-", gender, "-o2.rds")))
...
ggsave(file.path("figures", country, variable, paste0("spatialEffect_", Gender_Cat, ".png")), 
       device = "png", width = 8, height = 6, dpi = 600)
...
cartography$number_resp <- as.numeric(table(factor(nuts, levels = 1:NNUTS)))
cartography$number_neig <- nadj

p1 <- ggplot(cartography, aes(x = number_resp, y = thetasd_ordinal)) + 
  geom_point() + geom_smooth(method = "loess") +
  labs(x = "Number of respondents", y = "Posterior SD") +
  theme_minimal()

p2 <- ggplot(cartography, aes(x = number_resp, y = thetasd_bernoulli1)) + 
  geom_point() + geom_smooth(method = "loess") +
  labs(x = "Number of respondents", y = "Posterior SD") +
  theme_minimal()

p3 <- ggplot(cartography, aes(x = number_resp, y = thetasd_bernoulli2)) + 
  geom_point() + geom_smooth(method = "loess") +
  labs(x = "Number of respondents", y = "Posterior SD") +
  theme_minimal()

p4 <- ggplot(cartography, aes(x = number_neig, y = thetasd_ordinal)) + 
  geom_point() + geom_smooth(method = "loess") +
  labs(x = "Number of neighbors", y = "Posterior SD") +
  theme_minimal()

p5 <- ggplot(cartography, aes(x = number_neig, y = thetasd_bernoulli1)) + 
  geom_point() + geom_smooth(method = "loess") +
  labs(x = "Number of neighbors", y = "Posterior SD") +
  theme_minimal()

p6 <- ggplot(cartography, aes(x = number_neig, y = thetasd_bernoulli2)) + 
  geom_point() + geom_smooth(method = "loess") +
  labs(x = "Number of neighbors", y = "Posterior SD") +
  theme_minimal()

p1 <- p1 + ggtitle("Ordinal")
p2 <- p2 + ggtitle("Bernoulli (D+)")
p3 <- p3 + ggtitle("Bernoulli (D-)")

tema_grid <- theme(plot.title = element_text(hjust = 0.5, face = "bold"),
                   plot.tag = element_text(face = "bold", angle = 90, size = 12),
                   plot.tag.position = c(0, 0.5),
                   plot.margin = margin(5.5, 5.5, 5.5, 20))

final_plot <- wrap_plots(p1 + tema_grid, p2 + tema_grid, p3 + tema_grid,
                         p4 + tema_grid, p5 + tema_grid, p6 + tema_grid, ncol = 3)
final_plot

ggsave(file.path("figures", country, variable, paste0("largeSD_", Gender_Cat, ".png")), 
       device = "png", width = 10, height = 6, dpi = 600)

variable; country; gender
...
ggsave(file.path("figures", country, variable, paste0("ordinal_profiles_", Gender_Cat, ".png")),
...
#### Descriptive analysis - With also gender ####

ordinal_survey <- survey
y <- as.numeric(ordinal_survey$y)
y[y > 5] <- NA
nuts_all <- factor(ordinal_survey$nuts, levels = 1:NNUTS)
table(nuts_all)
...
ggsave(file.path("figures", country, variable, "descriptive.png"), 
       device = "png", width = 8, height = 6, dpi = 600)

#### Data preparation - With also gender ####
...
# Number of levels of each (categorical) covariate
NGnd <- length(table(survey$gnd))
NAge <- length(table(survey$age))
NEdu <- length(table(survey$edu))

#### Population loading - With also gender ####
...
#### Additional work: models with also gender ####
...
modelCode <- nimbleCode(
  {
    ...
    for (Resp in 1:NResp) {
      ...
      # Linear predictor
      for (Cat in 1:(NCats-1)) {
        logit(p.gamma[Resp, Cat]) <- kappa[Cat] - beta_gnd[gnd[Resp]] -
          beta_age[age[Resp]] - beta_edu[edu[Resp]] - 
          sd.theta * theta[nuts[Resp]]
      }
    }
    ...
    # beta_gnd[1:NGnd] gender fixed effect (corner constraint)
    beta_gnd[1] <- 0
    for (Gender in 2:NGnd) {
      beta_gnd[Gender] ~ dflat()
    }
...
modelConstants <- list(gnd = gnd, age = age, edu = edu, nuts = nuts,
                       NResp = NResp, NCats = NCats, NAge = NAge,
                       NGnd = NGnd, NEdu = NEdu, ones = ones, NNUTS = NNUTS,
                       NDist = NDist, Lambda = Lambda, from.to = from.to)

### Parameters to be saved ###

modelParameters <- c("kappa", "beta_gnd", "beta_age", "beta_edu",
                     "theta", "sd.theta", "rho")
...
modelInits <- local({
  ...
  function() {
    library(extraDistr)
    NGnd <- constants$NGnd; NAge <- constants$NAge
    NEdu <- constants$NEdu; NNUTS <- constants$NNUTS
    NCats <- constants$NCats; ones <- constants$ones
    list(delta = as.numeric(rdirichlet(1, ones)),
         beta_gnd = c(NA, rnorm(NGnd - 1)),
         beta_age = c(NA, rnorm(NAge - 1)),
         beta_edu = c(NA, rnorm(NEdu - 1)),
         rho = runif(1), sd.theta = runif(1),
         theta = rnorm(NNUTS, sd = 0.1))
...
saveRDS(salnimble, file = file.path("results", country, variable, "ordinal-leroux-hmc-waic.rds"))
...
modelCode <- nimbleCode(
  {
    ...
    for (Resp in 1:NResp) {
      ...
      # Linear predictor  
      logit(prsuccess[Resp]) <- beta_0 + beta_gnd[gnd[Resp]] +
        beta_age[age[Resp]] + beta_edu[edu[Resp]] + 
        sd.theta * theta[nuts[Resp]]
    }
...
modelConstants <- list(gnd = gnd, age = age, edu = edu, nuts = nuts,
                       NResp = NResp, NGnd = NGnd, NAge = NAge,
                       NEdu = NEdu, NNUTS = NNUTS,
                       NDist = NDist, Lambda = Lambda, from.to = from.to)

### Parameters to be saved ###

modelParameters <- c("beta_0", "beta_gnd", "beta_age", "beta_edu",
                     "theta", "sd.theta", "rho")
...
saveRDS(salnimble, file = file.path("results", country, variable, "bernoulli-leroux-hmc-waic-o1.rds"))

#### Model results (with also gender) ####

# Ordinal regression
ordinal_results <- readRDS(file = file.path("results", country, variable, "ordinal-leroux-hmc-waic.rds"))
# Logistic regression (Option 1): 0 = First two categories; 1 = Last three categories.
bernoulli_results1 <- readRDS(file = file.path("results", country, variable, "bernoulli-leroux-hmc-waic-o1.rds"))
# Logistic regression (Option 2): 0 = First three categories; 1 = Last two categories.
bernoulli_results2 <- readRDS(file = file.path("results", country, variable, "bernoulli-leroux-hmc-waic-o2.rds"))
...
NimToWin <- function(salnimble) {
  ...
  kappa <- matrix(nrow = nsims, ncol = NCats - 1)
  beta_gnd <- matrix(nrow = nsims, ncol = NGnd)
  beta_age <- matrix(nrow = nsims, ncol = NAge)
  beta_edu <- matrix(nrow = nsims, ncol = NEdu)
  theta <- matrix(nrow = nsims, ncol = NNUTS)
  sd.theta <- numeric(length = nsims)
  ...
  for (Gender in 1:NGnd) {
    beta_gnd[, Gender] <- c(salnimble[[1]][, paste0("beta_gnd[", Gender, "]")], 
                            salnimble[[2]][, paste0("beta_gnd[", Gender, "]")], 
                            salnimble[[3]][, paste0("beta_gnd[", Gender, "]")],
                            salnimble[[4]][, paste0("beta_gnd[", Gender, "]")],
                            salnimble[[5]][, paste0("beta_gnd[", Gender, "]")])
  }
  ...
  summary <- MCMCsummary(object = salnimble, round = 4)
  sims.list <- list("kappa" = kappa, "beta_gnd" = beta_gnd, 
                    "beta_age" = beta_age, "beta_edu" = beta_edu,
                    "theta" = theta, "sd.theta" = sd.theta, "rho" = rho)
...
NimToWin <- function(salnimble) {
  ...
  summary <- MCMCsummary(object = salnimble, round = 4)
  sims.list <- list("beta_0" = beta_0, "beta_gnd" = beta_gnd, 
                    "beta_age" = beta_age, "beta_edu" = beta_edu,
                    "theta" = theta, "sd.theta" = sd.theta, "rho" = rho)
...
#### Covariate effects (with also gender) ####

df_plot_ord <- data.frame("param" = rownames(ordinal_results$summary)[1:8], 
                          "mean" = ordinal_results$summary[1:8, 1],
                          "lower" = ordinal_results$summary[1:8, 3],
                          "upper" = ordinal_results$summary[1:8, 5],
                          "model" = "Ordinal")
df_plot_bern1 <- data.frame("param" = rownames(bernoulli_results1$summary)[2:9], 
                            "mean" = bernoulli_results1$summary[2:9, 1],
                            "lower" = bernoulli_results1$summary[2:9, 3],
                            "upper" = bernoulli_results1$summary[2:9, 5],
                            "model" = "Bernoulli (D+)")
df_plot_bern2 <- data.frame("param" = rownames(bernoulli_results2$summary)[2:9], 
                            "mean" = bernoulli_results2$summary[2:9, 1],
                            "lower" = bernoulli_results2$summary[2:9, 3],
                            "upper" = bernoulli_results2$summary[2:9, 5],
                            "model" = "Bernoulli (D-)")
df_plot <- rbind(df_plot_ord, df_plot_bern1, df_plot_bern2)
df_plot$param <- factor(df_plot$param, levels = rev(df_plot$param[1:8]))
df_plot$model <- factor(df_plot$model, levels = c("Ordinal", "Bernoulli (D+)", "Bernoulli (D-)"))
...
ggsave(file.path("figures", country, variable, "fixed.png"), 
       device = "png", width = 8, height = 6, dpi = 600)

#### Spatial effect (with also gender) ####
...
ggsave(file.path("figures", country, variable, "spatialEffect.png"), 
       device = "png", width = 8, height = 6, dpi = 600)

#### Post-stratification (with also gender) ####
...
poststratify <- function(salwinbugs) {
  ...
  p.gamma <- array(dim = c(nsims, NNUTS, NGnd, NAge, NEdu, NCats - 1))
  prlevels <- array(dim = c(nsims, NNUTS, NGnd, NAge, NEdu, NCats))
  prlevels_post <- array(dim = c(nsims, NNUTS, NCats))
  population <- population_eu
    ...
    for (Gender in 1:NGnd) {
      for (AgeGroup in 1:NAge) {
        for (EduGroup in 1:NEdu) {
          for (NUTS in 1:NNUTS) {
            for (Cat in 1:(NCats - 1)) {
              p.gamma[sim, NUTS, Gender, AgeGroup, EduGroup, Cat] <- 
                ilogit(salwinbugs$sims.list$kappa[sim, Cat] - 
                         salwinbugs$sims.list$beta_gnd[sim, Gender] - 
                         salwinbugs$sims.list$beta_age[sim, AgeGroup] - 
                         salwinbugs$sims.list$beta_edu[sim, EduGroup] - 
                         salwinbugs$sims.list$sd.theta[sim] * salwinbugs$sims.list$theta[sim, NUTS])
            }
            
            prlevels[sim, NUTS, Gender, AgeGroup, EduGroup, 1] <- p.gamma[sim, NUTS, Gender, AgeGroup, EduGroup, 1]
            prlevels[sim, NUTS, Gender, AgeGroup, EduGroup, NCats] <- 1 - p.gamma[sim, NUTS, Gender, AgeGroup, EduGroup, NCats - 1]
            
            for (Cat in 2:(NCats - 1)) {
              prlevels[sim, NUTS, Gender, AgeGroup, EduGroup, Cat] <- 
                p.gamma[sim, NUTS, Gender, AgeGroup, EduGroup, Cat] - p.gamma[sim, NUTS, Gender, AgeGroup, EduGroup, Cat-1]
            }
          }
        }
      }
...
poststratify <- function(salwinbugs) {
  ...
  prsuccess <- array(dim = c(nsims, NNUTS, NGnd, NAge, NEdu))
  prsuccess_post <- matrix(nrow = nsims, ncol = NNUTS)
  population <- population_eu
  ...
  for (sim in 1:nsims) {
    for (Gender in 1:NGnd) {
      for (AgeGroup in 1:NAge) {
        for (EduGroup in 1:NEdu) {
          for (NUTS in 1:NNUTS) {
            prsuccess[sim, NUTS, Gender, AgeGroup, EduGroup] <- 
              ilogit(salwinbugs$sims.list$beta_0[sim] + 
                       salwinbugs$sims.list$beta_gnd[sim, Gender] +
                       salwinbugs$sims.list$beta_age[sim, AgeGroup] + 
                       salwinbugs$sims.list$beta_edu[sim, EduGroup] + 
...
ggsave(file.path("figures", country, variable, "prevalenceOrdinal.png"), 
       device = "png", width = 15, height = 10, dpi = 600)
...
ggsave(file.path("figures", country, variable, "prevalenceDplus.png"), 
       device = "png", width = 8, height = 6, dpi = 600)
...
ggsave(file.path("figures", country, variable, "prevalenceDminus.png"), 
       device = "png", width = 8, height = 6, dpi = 600)
...
ggsave(file.path("figures", country, variable, "comparison.png"), 
       device = "png", width = 8, height = 6, dpi = 600)
...
ggsave(file.path("figures", country, variable, "largeSD.png"), 
       device = "png", width = 10, height = 6, dpi = 600)
